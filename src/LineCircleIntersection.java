...
public class LineCircleIntersection {
 ...
 //rounds a negative double away from 0 and a positive double towards zero
    ...
    public static List<Point2D> intersection(
    		Point2D secantP1, // earth coordinates
    		Point2D secantP2, // from equation for secant 
    		Point2D solarCenter,// (505,505)
            double radius, // Saturn's distance from sun
            boolean isSegment) //TRUE
            		throws NoninvertibleTransformException {
        List<Point2D> result = new ArrayList<>();
        double dx = secantP2.getX() - secantP1.getX();
        double dy = secantP2.getY() - secantP1.getY();
        AffineTransform trans = AffineTransform.getRotateInstance(dx, dy);
        trans.invert();
        trans.translate(-solarCenter.getX(), -solarCenter.getY());
        Point2D p1a = trans.transform(secantP1, null);
        Point2D p2a = trans.transform(secantP2, null);
        double y = p1a.getY();
